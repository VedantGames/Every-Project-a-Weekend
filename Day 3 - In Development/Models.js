...
var Model = function (gl, verticies, indeces, normals, color) {
  this.vbo = gl.createBuffer();
  this.ibo = gl.createBuffer();
  this.nbo = gl.createBuffer();
  this.nPoint = indeces.length;
...
var CreateShaderProgram = function (gl, vsText, fsText) {
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    return {
      error: 'Unable to initialize the shader program:'+ gl.getProgramInfoLog(program)
    }
  }
  ...
  if (!gl.getProgramParameter(program, gl.VALIDATE_STATUS)) {
    return {
      error: 'Unable to validate shader program:'+ gl.getProgramInfoLog(program)
    }
  }
